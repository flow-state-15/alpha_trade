...
import { updateWatchlist, removeWatchlist, loadWatchlists } from "../../store/watchlists"
      ...
      {showModal && (
        ...
        <div>
            <form className="sb-form-vertical" onSubmit={handleSubmit}>
                <input value={name} onChange={(e) => setName(e.target.value)} autoFocus={true}/>
                <span onClick={() => setShowModal(false)}>cancel</span>
                <button disabled={name === ''} type="submit">rename watchlist</button>
            </form>
        </div>
      )}
    </>
